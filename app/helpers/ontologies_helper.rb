module OntologiesHelper
  ...
  def visits_data
    ...
    years = @analytics[@ontology.acronym.to_sym].to_h.keys.map {|e| e.to_s.to_i}.select {|e| e > 0}.sort.reverse
    now = Time.now
    years.each do |year|
      months = @analytics[@ontology.acronym.to_sym].to_h[year.to_s.to_sym].to_h.keys.map {|e| e.to_s.to_i}.select {|e| e > 0}.sort.reverse
