...
function bp_cls_link(cls_id, ont_acronym) {
  return (
    bp_ont_link(ont_acronym) +
    "?p=classes&conceptid=" +
    encodeURIComponent(cls_id)
  );
}
...
var ajax_process_interportal_cls_interval = null,
...
var ajax_process_ont_init = function () {
  ...
  ajax_process_ont_interval = window.setInterval(
    ajax_process_ont,
    ajax_process_timing
  );
};
...
var ajax_process_ont = function () {
  ...
  var ajax_uri =
    "/ajax/json_ontology/?ontology=" + encodeURIComponent(ont_acronym);
  jQuery.ajax({
    url: ajax_uri,
    ...
    success: function (data) {
      if (typeof data !== "undefined" && data.hasOwnProperty("name")) {
        ...
        jQuery('a[href="/ontologies/' + data.acronym + '"]').each(function (
          i,
          e
        ) {
...
var ajax_process_interportal_cls_init = function () {
  ...
  ajax_process_interportal_cls_interval = window.setInterval(
    ajax_process_interportal_cls,
    ajax_process_timing
  );
};
...
var ajax_process_interportal_cls = function () {
  ...
  var ajax_uri =
    linkA.attr("data-cls") +
    jQuery(document).data().bp.config.interportal_hash[portal_acronym].apikey;
  jQuery.ajax({
    url: ajax_uri,
    timeout: ajax_process_timeout * 1000,
    success: function (data) {
