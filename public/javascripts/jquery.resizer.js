jQuery.fn.extend({
  resize: function(params){
    ...
    return this.each(function(){
			...
			$(this).mouseup(function(e){clicked = false;$(this).css('cursor','default');});
			$("body").mouseup(function(e){clicked = false;$(this).css('cursor','default');});
			$("body").mousemove(function(e){
				if(clicked){
					var mouse_x = Math.round(e.pageX - $(target).eq(0).offset().left) - start_x;
					var mouse_y = Math.round(e.pageY - $(target).eq(0).offset().top) - start_y;
					...
					start_x = Math.round(e.pageX - $(target).eq(0).offset().left);
					start_y = Math.round(e.pageY - $(target).eq(0).offset().top);
				}
			});					
    });
  }
